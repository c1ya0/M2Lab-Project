...
# 禁用 torch._dynamo 和 torch.compile（如果使用系統 Python 3.13 時的防禦措施）
# 注意：如果正確使用 conda 環境（Python 3.11 + PyTorch 2.4.0），理論上不需要這個設置
# 但保留作為防禦性措施，防止 subprocess 中環境變量設置失敗
...
class AEGNNDDPTrainer:
    ...
    def __init__(self, 
        ...
        if scheduler_type == 'cosine':
            self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                self.optimizer,
                T_max=1000,  # Will be updated in train()
                eta_min=min_lr
            )
    ...
    def train(self, num_epochs=200, save_dir='./checkpoints', resume_from=None):
        ...
        # Update cosine scheduler T_max if using cosine annealing
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if self.scheduler_type != 'onecycle' and epoch < self.warmup_epochs:
...
def train_worker(rank, world_size, args, master_port=None):
    ...
    if args.tdc_dataset:
        ...
        # Allow overriding processed TDC directory (default keeps backward compatibility)
        base_dir = getattr(args, "tdc_processed_dir", "data/processed_tdc_data")
        tdc_data_dir = os.path.join(base_dir, args.tdc_dataset, f"seed{args.tdc_seed}")
...
def main():
    ...
    parser.add_argument('--tdc_dataset', type=str, default=None,
                       help='TDC dataset name (e.g., caco2_wang). If provided, will load from {tdc_processed_dir}/{dataset}/seed{seed}/')
