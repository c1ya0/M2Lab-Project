...
class AEGNNM(nn.Module):
    ...
    def __init__(self, 
                 ...
                 pre_norm=False,
                 rotate_aug=False,
                 rotation_prob=0.5,  # IMPROVEMENT 5.1: Probability of applying rotation (0.0-1.0)
                 max_rotation_angle=180.0,  # IMPROVEMENT 5.1: Maximum rotation angle in degrees (0.0-180.0)
        ...
        self.rotation_prob = rotation_prob  # IMPROVEMENT 5.1
        self.max_rotation_angle = max_rotation_angle  # IMPROVEMENT 5.1
        ...
        # Original version: Use output_norm (LayerNorm) before output_proj
    ...
    def forward(self, x, edge_index, edge_attr, batch=None, pos=None, fingerprint=None, descriptor=None, return_graph_features=False, b2revb=None):
        ...
        # Apply 3D rotation augmentation if enabled and in training mode (IMPROVEMENT 5.1: with probability control)
        if self.rotate_aug and self.training and pos is not None:
            # Apply rotation with probability rotation_prob
            if torch.rand(1, device=pos.device).item() < self.rotation_prob:
    ...
    def _get_random_rotation_matrix(self, dtype=torch.float32, device=None):
        """Get a random 3D rotation matrix (IMPROVEMENT 5.1: with max_rotation_angle control)"""
        ...
        # Random rotation angle (IMPROVEMENT 5.1: limited by max_rotation_angle)
        max_angle_rad = math.radians(self.max_rotation_angle)
        theta = torch.rand(1, device=device, dtype=dtype) * (2 * max_angle_rad)
    ...
    def project_graph_features(self, graph_features):
        ...
        # Original version: Apply output_norm before output_proj
