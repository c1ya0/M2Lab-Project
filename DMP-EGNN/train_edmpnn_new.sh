...
# ============================================================================
# AEGNN-M Training Script (NEW Version - Fusion Model Logic)
# PARALLEL VERSION - Seeds run concurrently on 2 GPUs
# ============================================================================
# 
# Features:
#   - Follows fusion_model's approach: All 5 seeds use the same best hyperparameters
#   - Automatically loads Optuna-optimized hyperparameters from optuna_mod_new results
#   - Uses train_edmpnn.py for training (DMPNN architecture)
#   - Trains each dataset with 5 seeds (1-5) in PARALLEL on 2 GPUs
#     * GPU 0: seeds 1, 2
#     * GPU 1: seeds 3, 4, 5
#   - All seeds share the same best hyperparameters (from Optuna optimization)
...
# scripts/train_edmpnn.py must be called with --tdc_processed_dir to read from it.
# -----------------------------------------------------------------------------
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "${SCRIPT_DIR}" || exit 1

TDC_PROCESSED_DIR="data/processed_tdc_data_new"

if [ ! -d "${TDC_PROCESSED_DIR}" ]; then
    echo "❌ New processed dir not found: ${TDC_PROCESSED_DIR}"
...
echo "🧪 AEGNN-M Optimized Training (NEW - Fusion Model Logic: Shared Best Hyperparameters)..."
echo "================================================================="
...
check_tdc_dataset() {
    ...
    local tdc_data_dir="${TDC_PROCESSED_DIR}/${dataset_name}"
...
load_optuna_mod_params() {
    local dataset_name=$1
    # NEW: No seed_num parameter - all seeds use the same best hyperparameters
    
    # Try to load from best_params.json file first (from optuna_mod_new results)
    local best_params_file="checkpoints/optuna_mod_new/${dataset_name}/best_trial_models/best_params.json"
    
    # If not found, try to load from optuna database directly
    if [ ! -f "${best_params_file}" ]; then
        echo -e "${YELLOW}⚠️  Best params file not found: ${best_params_file}${NC}"
        echo -e "${BLUE}   Attempting to load from Optuna database...${NC}"
        
        # Try to extract best params from Optuna database
        local storage_url="sqlite:///optuna_edmpnn_results/optuna_mod_new.db"
        local study_name="aegnn_mod_${dataset_name}_opt"
        
        best_params_file=$(mktemp)
        python3 - <<PY
import optuna
import json
import sys
...
try:
    ...
    print(f"ERROR: Failed to load from Optuna database: {e}", file=sys.stderr)
    sys.exit(1)
PY
        
        if [ $? -ne 0 ]; then
            echo -e "${RED}❌ Failed to load best hyperparameters from Optuna database${NC}"
            rm -f "${best_params_file}"
            return 1
        fi
    fi
...
try:
    ...
    print(f"ERROR: Failed to read best params file: {e}", file=sys.stderr)
    sys.exit(1)
...
PY
    ...
    if [[ "${best_params_file}" == /tmp/* ]]; then
        rm -f "${best_params_file}"
    fi
    
    # ------------------------------------------------------------------
    # Safety check: num_heads must be divisible by hidden_dim / out_channels
    # Old version train_edmpnn.py will also check in GATEGNNLayer
    #   AssertionError: out_channels must be divisible by heads
    # Here we automatically adjust at shell side first, avoid direct error interrupting entire dataset
    # ------------------------------------------------------------------
    if [ -n "${hidden_dim}" ] && [ -n "${num_heads}" ]; then
        if [[ "${hidden_dim}" =~ ^[0-9]+$ ]] && [[ "${num_heads}" =~ ^[0-9]+$ ]] && [ "${num_heads}" -gt 0 ]; then
            ...
            if [ "${mod_val}" -ne 0 ]; then
                ...
                echo -e "${YELLOW}⚠️  Adjusted num_heads for ${dataset_name}: ${original_heads} -> ${num_heads} (hidden_dim=${hidden_dim}) to satisfy 'out_channels % heads == 0'${NC}"
            fi
        fi
    fi
...
train_dataset() {
    ...
    # NEW: Load best hyperparameters once (all seeds will use the same hyperparameters)
    # This follows fusion_model's approach: all seeds share the same best hyperparameters
    if ! load_optuna_mod_params "${dataset_name}"; then
        echo -e "${RED}❌ Failed to load best hyperparameters for ${dataset_name}${NC}"
        return 1
    fi
    
    echo -e "${BLUE}🚀 Starting Training (Fusion Model Logic): ${dataset_name} (Shared Best Hyperparameters, 5 Seeds - PARALLEL)${NC}"
    echo -e "   Task Type: ${task_type}"
    echo -e "   Primary Metric: ${GREEN}${primary_metric}${NC}"
    echo -e "   ${BLUE}Seeding Mode: ${GREEN}${SEEDING_MODE}${NC} (Model Init: $([ "$SEEDING_MODE" = "fusion_model" ] && echo "seed directly (1-5)" || echo "seed * 1000 + seed (1001-5005)"))"
    if [ "${task_type}" = "regressor" ] && [ "${primary_metric}" = "spearman" ]; then
        echo -e "   ${YELLOW}⚠️  Note: Using Spearman correlation - ensure train_edmpnn.py uses SpearmanLoss${NC}"
    fi
    echo -e "   ${GREEN}⚡ Parallel Mode: Seeds will run concurrently on 2 GPUs${NC}"
    echo -e "   ${GREEN}📋 All seeds will use the same best hyperparameters (Fusion Model Logic)${NC}"
    ...
    train_single_seed() {
        ...
        local seed_data_dir="${TDC_PROCESSED_DIR}/${dataset_name}/seed${seed}"
        if [ ! -d "${seed_data_dir}" ] || \
        ...
        # NEW: Hyperparameters are already loaded in train_dataset function
        # All seeds use the same best hyperparameters (Fusion Model Logic)
        
        # Model initialization seed: depends on seeding mode
        # - "dmp_egnn" (default): seed * 1000 + seed (e.g., seed 1 → 1001, seed 2 → 2002)
        # - "fusion_model": use seed directly (e.g., seed 1 → 1, seed 2 → 2)
        # This allows comparability with fusion_model while preserving existing results
        if [ "$SEEDING_MODE" = "fusion_model" ]; then
            local model_init_seed=$seed
        else
            # Default: DMP-EGNN mode
        ...
        echo -e "   Model Init Seed: ${model_init_seed} (${SEEDING_MODE} mode)"
        echo -e "   Using shared best hyperparameters (Fusion Model Logic)"
        ...
        local train_cmd=(
            ...
            --tdc_seed "${seed}"
            --tdc_processed_dir "${TDC_PROCESSED_DIR}"
            ...
            --min_lr "${min_lr}"
            # Note: Using 200 epochs for final training (vs 100 epochs in Optuna search)
            # This allows models to train longer and potentially achieve better performance
            # Early stopping will still terminate training early if no improvement is detected
            --num_epochs 200
        ...
        # train_edmpnn.py now supports single GPU mode (non-DDP)
...
show_help() {
    ...
    echo ""
    echo "AEGNN-M Training Script (NEW - Fusion Model Logic)"
    echo "Trains each dataset with 5 seeds (1-5) using shared best Optuna-optimized hyperparameters"
    echo ""
    echo "Options:"
    ...
    echo ""
    echo "Examples:"
    echo "  $0                                    # Train all 22 TDC datasets (dmp_egnn mode)"
    echo "  $0 --seeding-mode fusion_model        # Train all datasets (fusion_model mode)"
    echo "  $0 ames bbb_martins caco2_wang       # Train specific datasets (dmp_egnn mode)"
    echo "  $0 ames --seeding-mode fusion_model  # Train ames (fusion_model mode)"
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --seeding-mode|--seeding_mode)
            shift
            if [ $# -eq 0 ]; then
                echo -e "${RED}❌ Missing seeding mode after --seeding-mode${NC}"
                echo "Use --help for usage information"
                exit 1
            fi
            if [ "$1" = "dmp_egnn" ] || [ "$1" = "fusion_model" ]; then
                SEEDING_MODE="$1"
            else
                echo -e "${RED}❌ Invalid seeding mode: $1${NC}"
                echo -e "${YELLOW}   Valid options: dmp_egnn, fusion_model${NC}"
                exit 1
            fi
            ;;
        *)
...
echo ""
echo -e "${GREEN}🎉 All training tasks completed! (Fusion Model Logic: Shared Best Hyperparameters)${NC}"
