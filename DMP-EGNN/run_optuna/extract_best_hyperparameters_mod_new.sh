...
# =============================================================================
# Wrapper script for extract_best_hyperparameters_mod_new.py
#
# 功能：
#   - 從 optuna_mod_new 的資料庫中，為多個 TDC dataset 抽取最佳超參數
#   - 會同時在：
#       1) 使用者指定的 --output-dir（若有）
#       2) checkpoints/optuna_mod_new/<dataset>/best_trial_models/best_trial_info.json
#     寫入結果（後者是 Python 腳本已經內建的行為）
#
# 使用方式：
#   1) 指定特定 datasets：
#        ./scripts/extract_best_hyperparameters_mod_new.sh ames herg caco2_wang
#
#   2) 全部 22 個 TDC datasets：
#        ./scripts/extract_best_hyperparameters_mod_new.sh --all
#      或（沒給任何 dataset 參數時，也會視為 --all）
#        ./scripts/extract_best_hyperparameters_mod_new.sh
#
#   3) all + exclude：
#        ./scripts/extract_best_hyperparameters_mod_new.sh --all \
...
#        ./scripts/extract_best_hyperparameters_mod_new.sh --all \
#            --storage sqlite:///optuna_edmpnn_results/optuna_mod_new.db \
...
STORAGE_PATH="sqlite:///optuna_edmpnn_results/optuna_mod_new.db"
# 預設輸出到 optuna_edmpnn_results，使得最終檔名為：
#   optuna_edmpnn_results/all_best_hyperparameters_mod_new.json
...
show_help() {
    ...
    echo ""
    echo "Wrapper for scripts/extract_best_hyperparameters_mod_new.py"
    echo ""
...
# 組合並執行 Python 指令
# ---------------------------
PY_CMD=(python3 scripts/extract_best_hyperparameters_mod_new.py)
PY_CMD+=("--dataset")
...
echo ""
echo "✅ 抽取完成。合併結果已寫入："
echo "   ${OUTPUT_DIR}/all_best_hyperparameters_mod_new.json"
