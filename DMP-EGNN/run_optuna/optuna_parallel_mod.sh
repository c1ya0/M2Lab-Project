...
# Optuna Parallel Optimization Launcher (MOD version for AEGNN-M with DMPNN)
# Usage: ./optuna_parallel_mod.sh [dataset1] [dataset2 ...] [mode: fast/standard/deep] [--exclude dataset1 dataset2 ...] [--workers N]
#        ./optuna_parallel_mod.sh all [mode] [--exclude dataset1 dataset2 ...] [--workers N]
# Supports multiple datasets: ./optuna_parallel_mod.sh herg ames bace standard
#
...
case $mode in
    ...
    standard)
        ...
        # Calculation: 50 trials × avg 275 min/trial (5 seeds per trial, ~55 min per seed) ÷ 2 workers = 6875 min ≈ 114.6 hours
        # Plus buffer: 114.6 × 1.5 = 171.9 hours (more conservative buffer due to longer training time)
        MAX_WAIT_TIME=619200  # Approx 172 hours (adjusted for 5 seeds per trial)
        ;;
    deep)
        TOTAL_TRIALS=160
...
run_optimization_for_seed() {
    ...
    STORAGE_URL="sqlite:///optuna_edmpnn_results/optuna_mod.db"
    echo "   Storage: $STORAGE_URL"
    echo "   Logs: logs/optuna_launcher_mod/"

    mkdir -p logs/optuna_launcher_mod
    mkdir -p optuna_edmpnn_results

    # Study name for per-seed optimization
    STUDY_NAME="aegnn_mod_${current_dataset}_seed${seed_num}_opt"
...
" 2>/dev/null || echo "0,0,0,0")
    ...
    for i in $(seq 1 $WORKERS); do
        ...
        if [ "$i" -eq 1 ]; then
            ...
            python3 scripts/optuna_serach_mod.py \
                ...
                --worker_id "$i" \
                --seed "$seed_num" \
                2>&1 | tee "logs/optuna_launcher_mod/worker_${current_dataset}_seed${seed_num}_${i}.log" &
            ...
            python_pid=$(pgrep -f "optuna_serach_mod.py.*--dataset.*$current_dataset.*--seed.*$seed_num.*--worker_id.*$i" | grep -v grep | head -1)
            if [ -n "$python_pid" ]; then
                worker_pid=$python_pid
            else
                # Fallback: try without worker_id (for backward compatibility)
                python_pid=$(pgrep -f "optuna_serach_mod.py.*--dataset.*$current_dataset.*--seed.*$seed_num" | grep -v grep | head -1)
                if [ -n "$python_pid" ]; then
                    worker_pid=$python_pid
                else
            ...
            if ! kill -0 "$worker_pid" 2>/dev/null; then
                ...
                if [ -f "logs/optuna_launcher_mod/worker_${current_dataset}_seed${seed_num}_${i}.log" ]; then
                    if grep -q "Error\|Traceback\|Exception" "logs/optuna_launcher_mod/worker_${current_dataset}_seed${seed_num}_${i}.log" 2>/dev/null; then
            ...
            setsid python3 scripts/optuna_serach_mod.py \
                ...
                --worker_id "$i" \
                --seed "$seed_num" \
                > "logs/optuna_launcher_mod/worker_${current_dataset}_seed${seed_num}_${i}.log" 2>&1 &
            local worker_pid=$!
...
run_optimization() {
    ...
    # Note: Study initialization is now handled by optuna_serach_mod.py
    ...
    echo "   Study will be initialized by optuna_serach_mod.py with proper direction detection"

    # Check existing trials and calculate remaining trials needed
    # Note: Study may not exist yet (will be created by optuna_serach_mod.py)
    echo "   Checking existing trials in study..."
    EXISTING_TRIALS_INFO=$(python3 -c "
import optuna
try:
    # Try to load existing study (will fail if study doesn't exist)
    study = optuna.load_study(study_name='aegnn_mod_${current_dataset}_opt', storage='$STORAGE_URL')
    total = len(study.trials)
    completed = len([t for t in study.trials if t.state == optuna.trial.TrialState.COMPLETE])
    failed = len([t for t in study.trials if t.state == optuna.trial.TrialState.FAIL])
    running = len([t for t in study.trials if t.state == optuna.trial.TrialState.RUNNING])
...
" 2>/dev/null || echo "0,0,0,0")
    ...
    for i in $(seq 1 $WORKERS); do
        ...
        if [ "$i" -eq 1 ]; then
            ...
            setsid python3 scripts/optuna_serach_mod.py \
                ...
                --epochs "$EPOCHS" \
                --worker_id "$i" \
                2>&1 | tee "logs/optuna_launcher_mod/worker_${current_dataset}_${i}.log" &
            local worker_pid=$!
            ...
            if ! kill -0 "$worker_pid" 2>/dev/null; then
                # Process already died, check log for errors
                if [ -f "logs/optuna_launcher_mod/worker_${current_dataset}_${i}.log" ]; then
                    if grep -q "Error\|Traceback\|Exception" "logs/optuna_launcher_mod/worker_${current_dataset}_${i}.log" 2>/dev/null; then
                        echo "   ⚠️  Worker $i failed to start (check log for errors)"
                        continue  # Skip this worker
                    fi
                fi
            ...
            setsid python3 scripts/optuna_serach_mod.py \
                ...
                --epochs "$EPOCHS" \
                --worker_id "$i" \
                > "logs/optuna_launcher_mod/worker_${current_dataset}_${i}.log" 2>&1 &
            local worker_pid=$!
    ...
    echo "   To monitor other workers, use: tail -f logs/optuna_launcher_mod/worker_${current_dataset}_2.log"
...
" 2>/dev/null || echo "0,0,0,0")
    ...
    if [ ${#FAILED_PIDS[@]} -gt 0 ]; then
        ...
        echo "   Check logs in logs/optuna_launcher_mod/ for details."
        return 1
    else
    ...
    if [ -d "$TDC_DATASET_PATH" ]; then
        ...
        if [ "$SEED_FOUND" = true ]; then
            ...
            for seed_num in 1 2 3 4 5; do
                ...
                if [ -d "$TDC_DATASET_PATH/seed${seed_num}" ] && \
                   ...
                   [ -f "$TDC_DATASET_PATH/seed${seed_num}/test.pt" ]; then
                    ...
                    seed_python_pids=$(pgrep -f "optuna_serach_mod.py.*--dataset.*$dataset.*--seed.*$seed_num" | grep -v grep || true)
            ...
            for i in "${!seed_pids[@]}"; do
                ...
                python_pids=$(pgrep -f "optuna_serach_mod.py.*--dataset.*$dataset.*--seed.*$seed_num" | grep -v grep || true)
