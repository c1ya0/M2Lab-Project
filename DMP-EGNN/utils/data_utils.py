...
class MolecularGraphBuilder:
    ...
    def _generate_3d_coordinates(self, mol, smiles: str = None) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if self.num_conformers > 1:
                result = self._generate_multiple_conformers(mol, smiles)
            else:
    ...
    def _generate_multiple_conformers(self, mol, smiles: str = None) -> Optional[np.ndarray]:
        """
        Use RDKit's native EmbedMultipleConfs to generate multiple conformers
        and select the optimized one (lowest energy)
        ...
        Returns:
            3D coordinate array [N, 3] of the best conformer
        """
        try:
            ...
            # Optimize all conformers and calculate energy (supports multi-threading)
            best_conf_id = None
            
            if self.optimize_conformers:
                # Prefer MMFF force field optimization and energy calculation (multi-threaded)
                # MMFF is more accurate but only applicable to organic molecules (does not support metal ions, etc.)
                best_conf_id = self._optimize_and_select_best_mmff(mol, ids)
                
                # If MMFF fails, try UFF force field (multi-threaded)
                # UFF has wider applicability but will produce warnings for special atom types (e.g., metal ions)
                if best_conf_id is None:
                    self.uff_fallback_count += 1
                    best_conf_id = self._optimize_and_select_best_uff(mol, ids)
                else:
                    self.mmff_success_count += 1
            
            # If optimization fails, select the first conformer
            if best_conf_id is None:
                best_conf_id = ids[0]
            
            # Extract coordinates of the best conformer
    ...
    def smiles_to_graph(self, smiles: str) -> Data:
        ...
        try:
            ...
            # Generate 3D coordinates (prefer MMFF, fallback to UFF if unavailable)
            pos = self._generate_3d_coordinates(mol, smiles=smiles)
            ...
            if pos is None or pos.shape[0] != len(node_features):
                # Return None to exclude this molecule from the dataset
                if smiles is not None:
                    # Failure reason should already be recorded in _generate_3d_coordinates
                    pass
                return None
            ...
            data_dict = {
                ...
                'pos': torch.tensor(pos, dtype=torch.float)  # 3D coordinates are required
            }
            
            if fingerprint is not None:
                data_dict['fingerprint'] = torch.tensor(fingerprint, dtype=torch.float).unsqueeze(0)  # Add batch dim [1, bits]
            
            if descriptor is not None:
                data_dict['descriptor'] = torch.tensor(descriptor, dtype=torch.float).unsqueeze(0)  # Add batch dim [1, dim]
